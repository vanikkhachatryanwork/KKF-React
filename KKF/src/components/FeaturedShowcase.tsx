  ...
  return (
    <section className="featured-showcase">
      ...
      <Swiper
        ...
        slidesPerView={1.08}
        breakpoints={{
          700: { slidesPerView: 1.65, spaceBetween: 24 },
          1100: { slidesPerView: 2.2, spaceBetween: 28 },
        }}
      >
        {works.slice(0, 5).map((work, index) => (
          <SwiperSlide key={work.id}>
            <Link
              to={`${prefix}/works/${work.slug}`}
              className="featured-slide"
            >
              <MediaImage media={work.featuredImage} eager={index < 2} />
              <span className="featured-slide__number">0{index + 1}</span>
              <div>
                <h3>{localize(work.title, locale)}</h3>
