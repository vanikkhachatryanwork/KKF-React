...
export function Gallery({ images, variant = 'wide' }: { images?: Media[]; variant?: 'wide' | 'coverflow' }) {
  ...
  return <>
    <div className={`gallery-stage gallery-stage--${variant}`}><div className="gallery-stage__top"><span>KK FURNITURE</span><span>{String(current + 1).padStart(2, '0')} / {String(images.length).padStart(2, '0')}</span></div>
    <Swiper className={`gallery gallery--${variant}`} modules={[A11y, EffectCoverflow, Keyboard, Navigation, Pagination]} navigation pagination={{ clickable: true }} keyboard={{ enabled: true }} loop={images.length > 2} onSlideChange={(swiper) => setCurrent(swiper.realIndex)} centeredSlides={variant === 'coverflow'} effect={variant === 'coverflow' ? 'coverflow' : 'slide'} coverflowEffect={{ rotate: 0, stretch: 18, depth: 180, modifier: 1.15, slideShadows: true }} slidesPerView={variant === 'coverflow' ? 'auto' : 1} spaceBetween={24}>
